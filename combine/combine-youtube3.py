...
if not os.path.exists(apple_folder):
    print(f"오류: {apple_folder} 폴더가 존재하지 않습니다!")
    exit(1)

for root, _, files in os.walk(apple_folder):
    for file in files:
        if file.endswith(".json"):
            file_path = os.path.join(root, file)

            # apple 폴더 JSON 데이터 로드
            try:
...
print("🎉 모든 apple 폴더의 파일을 업데이트 완료!")
